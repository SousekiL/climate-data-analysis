...
## get data
shum_df_all <- data.frame()
for (i in 1:dim(city_loc)[1]) {
  
  svMisc::progress(i, dim(city_loc)[1])
  
  .toolik_lon <- city_loc$lon[i]
  .toolik_lat <- city_loc$lat[i]
  .toolik_series <-
    extract(r_brick, SpatialPoints(cbind(.toolik_lon, .toolik_lat)), method =
              'simple')
  .toolik_df <-
    data.frame(
      day = seq(ymd('2018-01-01'), ymd('2018-12-31'), by = '1 day'),
      humidity = t(.toolik_series),
      city = city_loc$addr[i],
      lon =  city_loc$lon[i],
      lat =  city_loc$lat[i]
    )
  shum_df_all <- rbind(shum_df_all, .toolik_df)
}
