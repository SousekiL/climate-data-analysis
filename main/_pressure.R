...
nc_data <-
  nc_open('data/pres_CMFD_V0106_B-01_01dy_010deg_201801-201812.nc')
# Save the print(nc) dump to a text file
{
  sink('data/pres_CMFD_V0106_B-01_01dy_010deg_201801-201812_meta.txt')
  print(nc_data)
  sink()
  }
...
ndvi.array <-
  ncvar_get(nc_data, "pres") # store the data in a 3-dimensional array
dim(ndvi.array)

fillvalue <- ncatt_get(nc_data, "pres", "_FillValue")
fillvalue
...
for (i in 1:dim(city_loc_list)[1]) {
  ...
  .toolik_df <-
    data.frame(
      ...
      pres = t(.toolik_series),
      city = city_loc_list$addr[i]
    )
  pres_df <- rbind(pres_df, .toolik_df)
}
...
pres_df %<>%
  ...
  group_by(city) %>%
  arrange(city, day) %>%
  mutate(pres_s = zoo::rollmean(pres, 7, na.pad = TRUE))